...
use rustique_core::config::manager::{get_config, init_config};
use rustique_core::errors::{ErrorMsgFn, handle_err_result};
use rustique_core::information_utils::{elapsed_footer, notice};
use rustique_core::options::RustiqueOptions;
...
fn one_click_setup() {
    ...
    let text =
        include_str!("rustique.desktop").replace("{RUSTIQUE_PATH}", &exe_path.to_string_lossy());
