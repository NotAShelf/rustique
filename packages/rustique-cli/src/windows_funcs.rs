...
use rustique_core::information_utils::{CellData, RustiqueMessage, notice, rustique_message};
    ...
    if let Some(app_data) = env::var_os("APPDATA") {
        ...
        if !mod_metadata.is_empty() {
            ...
            if !can_proceed {
                println!();
                notice(
                    "Ok, Rustique will not update your mod location. To prevent Rustique from checking again, use the following command:",
                    Some(Color::Green),
                    vec![],
                );
                notice(
                    "Rustique config set --update-default-windows-loc false",
                    Some(Color::Cyan),
                    vec![Attribute::Bold],
                );
                return Ok(());
            ...
            if !enabled_modpacks.is_empty() {
                if !is_elevated() {
                    notice(
                        "You have modpacks enabled. Rustique will need admin right to disable, then enable your modpacks.",
                        Some(Color::Yellow),
                        vec![Attribute::Bold],
                    );
                    exit(0);
                ...
                for mpk_id in &enabled_modpacks {
                    match mp_disable(mpk_id.clone(), &old_default).await {
                        ...
                        Err(e) => {
                            error!(
                                "Rustique was enable to disable your modpack {mpk_id} and cannot continue: {e}"
                            );
                            exit(1);
                        }
                    }
            ...
            if let Err(e) = iterate_and_move_zip(&mut mods, &new_default, true).await {
                notice(
                    format!(
                        "Rustique ran into errors while attempting to move your mods. You will need to move them manually, then use the following command to reset the default dir to the new one: {e}"
                    ),
                    Some(Color::Red),
                    vec![Attribute::Bold],
                );
                notice(
                    "Rustique config del -m:",
                    Some(Color::Cyan),
                    vec![Attribute::Bold],
                );
                exit(1);
            ...
            for mpk_id in &enabled_modpacks {
                match mp_enable(mpk_id.clone(), &new_default, true).await {
                    ...
                    Err(e) => {
                        notice(
                            format!(
                                "Rustique was enable to enable your modpack {mpk_id}, try using Rustique enable {mpk_id} instead: {e}"
                            ),
                            Some(Color::Red),
                            vec![Attribute::Bold],
                        );
        ...
        info!(
            "Unable to read the appdata folder. This should not happen and will cause errors with rustique"
        );
        return Ok(());
    }
