...
pub async fn iterate_and_delete(
    ...
    while let Some(entry) = mods
        .next_entry()
        .await
        .map_err(|e| RustiqueError::SimpleError(format!("Unable to iterate on dir: {e}")))?
    {
        ...
        .map_err(|e| RustiqueError::SimpleError(format!("Unable to iterate on dir: {e}")))?
    {
        if entry.path().exists() {
            ...
            tokio::fs::rename(&entry.path(), target_dir.as_ref().join(&file_name))
                .await
                .map_err(|e| RustiqueError::SimpleError(format!("Unable to move file: {e}")))?;
        }
    }
...
pub async fn delete_cmd(
    ...
    if !processed_mods.is_empty() {
        ...
        sync_data
            .save(Path::new(mod_dir).join(FILE_RUSTIQUE_SYNC))
            .await?;
    }
