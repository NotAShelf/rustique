...
async fn set(args: &CommonArgs) {
    ...
    if let Some(with_mod) = &args.with_mod {
        if let Some(version) = &args.pin_version {
            ...
            notice(
                "Be sure to run the sync command to update Rustique's sync file to use the newly set pinned mod version.",
                Some(Color::Green),
                vec![],
            );
        }
...
async fn del(args: &DelArgs) {
    ...
    if args.pinned_mod.is_some() {
        let Some(mod_id) = &args.pinned_mod else {
            warn!(
                "You must provide a Mod ID before anything can be removed. Run [./Rustique config list] to show all valid options"
            );
            exit(1);
        };
