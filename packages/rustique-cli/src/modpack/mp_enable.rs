...
pub async fn mp_enable(
    ...
    if !is_elevated() {
        notice(
            "In order to enable modpacks, Rustique uses symlinks which require admin permissions on Windows. Please run Rustique with admins right and try again.",
            Some(Color::Red),
            vec![Attribute::Bold],
        );
        exit(1);
    ...
    if !full_dir_with_mpk_id.exists() {
        return Err(RustiqueError::SimpleError(format!(
            "Modpack {} doesn't exist in {}. Run 'Rustique modpack list' or 'Rustique modpack local list' to view your installed modpacks.",
            &mpk_id,
