...
pub async fn mp_disable(mpk_id: ModID, mod_dir: impl PathRef) -> Result<ModID, RustiqueError> {
    ...
    if !is_elevated() {
        notice(
            "In order to disable modpacks, Rustique uses symlinks which require admin permissions on Windows. Please run Rustique with admin rights and try again.",
            Some(Color::Red),
            vec![Attribute::Bold],
        );
        exit(1);
    ...
    if !mod_pack_dir.exists() {
        return Err(RustiqueError::SimpleError(
            "Modpack {} doesn't exist. Run 'Rustique modpack list' to view installed modpacks."
                .into(),
        ));
    }
