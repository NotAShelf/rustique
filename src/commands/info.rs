...
pub async fn info(args: &ModInfoArgs) -> Result<(), RustiqueError> {
    ...
    let headers = vec!{
        prep_cell(&mod_info.mod_id.clone().to_string(), Some(CellColor::Magenta), Some(CellAttr::Bold), None, None),
    ...
    t1_rows.push(
        Row::from(vec![
            ...
            prep_cell(&format!("https://mods.vintagestory.at/show/mod/{}", &mod_info.asset_id.clone()), Some(CellColor::Yellow), None, None, None),
        ]),
    );
    ...
    t1_rows.push(Row::from(vec![
        ...
        prep_cell(&mod_info.downloads.clone().to_string(), Some(CellColor::Magenta), None, None, None)
    ]));

    t1_rows.push(Row::from(vec![
        prep_cell("Follows", Some(CellColor::Green), Some(CellAttr::Bold), None, None ),
        prep_cell(&mod_info.follows.clone().to_string(), Some(CellColor::Magenta), None, None, None)
    ]));
    
    t1_rows.push(Row::from(vec![
        prep_cell("Comments", Some(CellColor::Green), Some(CellAttr::Bold), None, None ),
        prep_cell(&mod_info.comments.clone().to_string(), Some(CellColor::Magenta), None, None, None)
    ]));
    ...
    if args.show_description {
        ...
        let t2_txt = html2text::from_read(&mut mod_info.text.clone().unwrap_or_default().as_bytes(), 100).map_err(|_| RustiqueError::SimpleError("html2txt failed".to_string()))?;
