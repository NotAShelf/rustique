...
use rustique_core::information_utils::{notice, rustique_message, CellData, RustiqueMessage};
...
pub async fn check_old_default_windows() -> Result<(), RustiqueError> {
    ...
    if let Some(app_data) = env::var_os("APPDATA") {
        ...
        info!("Unable to read the appdata folder. This should not happen and will cause errors with rustique");
        return Ok(())
    }
